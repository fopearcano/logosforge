...
def _detect_disappearance(scenes, characters, scene_char_map, total) -> Insight | None:
    ...
    if worst_name:
        ...
        return Insight(
            text=f"{worst_name} disappears for a long stretch.",
            severity=severity,
...
def _detect_stagnation(scenes, scene_char_map, total) -> Insight | None:
    ...
    if outer_diversity > 0:
        ...
        if ratio < 0.5:
            ...
            return Insight(
                text="Middle section lacks variation.",
                severity=severity,
                category="stagnation",
            )
    return None
...
def _detect_arc_neglect(scenes, total) -> Insight | None:
    ...
    if worst_pl:
        return Insight(
            text=f"Arc \"{worst_pl}\" is underdeveloped.",
...
def _detect_clustering(scenes, characters, scene_char_map, total) -> Insight | None:
    ...
    if worst_name:
        return Insight(
            text=f"{worst_name} only appears in a brief cluster.",
            severity=worst_score,
            category="clustering",
        )
    return None
...
def insight_color(category: str) -> str:
    """Map insight category to a soft indicator color."""
    colors = {
        "disappearance": "#f59e0b",
        "monotony": "#f59e0b",
        "stagnation": "#ef4444",
        "neglect": "#f59e0b",
        "clustering": "#6366f1",
