...
Unlike :mod:`storyplanner.export` (which produces *manuscript* text in formats
...
* :func:`build_full_export` — everything, in an *import-compatible* shape that
  round-trips through :mod:`storyplanner.import_data`.

All builders read live from the :class:`~storyplanner.db.Database`, which is the
...
def build_full_export(db: Database, project_id: int) -> dict:
    ...
    Reuses :func:`storyplanner.export._gather_project_data` so the result keeps
    the flat keys (``project``/``characters``/``places``/``notes``/``scenes``/
    ``psyke_entries``/``outline``/``continuity``) that
    :func:`storyplanner.import_data.import_json` understands, then layers on the
