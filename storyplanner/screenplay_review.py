...
def build_screenplay_review(db, project_id: int) -> ScreenplayReviewReport:
    ...
    try:
        from storyplanner.screenplay_continuity import build_screenplay_continuity_report
    ...
    try:
        from storyplanner.screenplay_diagnostics import _psyke_character_map
    ...
    try:
        from storyplanner.screenplay_interchange import validate_fountain_export_readiness
    ...
    for entry_idx, sid in enumerate(order):
        ...
        if not has_plan:
            try:
                from storyplanner.screenplay_pipeline import has_beat_plan
        ...
        if content.strip():
            try:
                from storyplanner import screenplay_blocks as _sb
        ...
        if content.strip():
            ...
            try:
                from storyplanner.screenplay_interchange import validate_export_blocks
