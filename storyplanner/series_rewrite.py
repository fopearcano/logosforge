...
* parse/serialize + validation reuse :mod:`storyplanner.series_blocks`;
* fence-stripping + leak phrases + the draft validation dataclass reuse
  :mod:`storyplanner.series_pipeline`;
...
def build_rewrite_request(
    ...
    if include_health:
        try:
            from storyplanner.series_diagnostics import analyze_scene_by_id
