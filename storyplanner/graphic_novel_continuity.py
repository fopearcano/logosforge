...
* setup/payoff + recurring objects (``storyplanner.screenplay_setup_payoff`` +
  confirmed ``db.get_story_links``),
* cross-scene continuity issues (``storyplanner.continuity``),
...
def _has_location(visual: str) -> bool:
    try:
        from storyplanner.graphic_novel_reflection import _has_location as _hl
        return _hl(visual)
    except Exception:
        return False
