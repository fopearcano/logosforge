...
(:mod:`storyplanner.screenplay_fountain`) and the canonical project export
(:mod:`storyplanner.export`). It adds what interchange needs end to end:
...
* **Import** — parse ``.fountain`` into *grouped scenes*, build a preview that
  ...
  scenes are created through :mod:`storyplanner.story_structure` (so they always
