...
* ``python -m storyplanner.diagnostics`` — prints the report with NO GUI. This is
  the fastest way to prove which ``storyplanner`` package Python actually resolves
...
Optional visible markers: set ``STORYPLANNER_DEV_MARKERS=1`` and each of the new
...
_DEV_MARKERS_ENV = "STORYPLANNER_DEV_MARKERS"
...
def runtime_report() -> dict:
    ...
    import storyplanner
    from storyplanner.ui import main_window as _mw
    ...
    return {
        ...
        "storyplanner_pkg": storyplanner.__file__,
...
def print_runtime_report(file=None) -> None:
    ...
    try:
        ...
        print(f"[storyplanner.diagnostics] report failed: {exc}", file=file)
        return
    lines = [
        "==== storyplanner runtime ====",
        f"  commit          : {r['commit']}",
        f"  python          : {r['python']}",
        f"  sys.path[:3]    : {r['sys_path_head']}",
        f"  package         : {r['storyplanner_pkg']}",
...
def attach_dev_marker(widget, text: str) -> None:
    """Float a tiny developer-only badge on *widget* when STORYPLANNER_DEV_MARKERS
