...
from storyplanner.models import STAGE_SCOPE_TYPES, STAGE_STATUSES
from storyplanner.stages import (
    SAFETY_STAGE_NAME,
    branch_from,
    capture_scope,
    diff_payloads,
...
class StagesView(QWidget):
    ...
    def _on_new_stage(self) -> None:
        ...
        if not ok or not name.strip():
            return
        scope_type, ok2 = QInputDialog.getItem(
            self, "New stage scope", "Scope:",
            list(_USER_SCOPES), 0, False,
        )
        if not ok2:
            return
        scope_id = None
    ...
    def _on_take_snapshot(self) -> None:
        ...
        save_snapshot(
            ...
            label=f"Snap {stage.created_at:%H:%M}",
