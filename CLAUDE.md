# CLAUDE.md — LogosForge Core (`fopearcano/storyplanner`)

> Canonical template:
> `fopearcano/logosforge-architecture` → `repo-templates/storyplanner/CLAUDE.md`.
...
- **Forbidden:** any frontend dependency — never import from, vendor, or
  ...
  Rule: `storyplanner API -> no frontend dependency`.

## Update rules

- Changes flow **downstream only**. Target cascade:
  `storyplanner → logosforge-ui-contracts → shared UI → apps` — but the
