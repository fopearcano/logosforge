...
business logic — every route delegates to existing `storyplanner` core services.
...
python -m storyplanner.api

# Or override on the CLI
python -m storyplanner.api --host 0.0.0.0 --port 9000 --mode lan --db ./storyplanner.db
```
...
```python
from storyplanner.api import create_api, ApiConfig
from storyplanner.db import Database

app = create_api(db=Database("storyplanner.db"), config=ApiConfig(mode="desktop"))
# hand `app` to uvicorn, or use storyplanner.api.server.run(config)
```
...
| `STORYPLANNER_DB_PATH` | `storyplanner.db` | SQLite project DB the API serves.              |
...
`storyplanner.connector_executor.execute_action`, which validates inputs against
...
* Response: `payload` (json), `content` (markdown text), or `files`
  (`{filename: csv_text}`). Reuses `storyplanner.data_export`.
...
The API owns its own Qt-free event broker (`storyplanner/api/events.py`); it
