...
Logos is StoryPlanner's **inline / contextual** AI layer. It is distinct from
...
## Phases (modules under `storyplanner/logos/`)
...
- **Core** (`context`, `actions`, `controller`, `prompt_builder`, `result`) —
  ...
  Professional Export) — read-only; DOCX/PDF/FDX file writes are export functions.
