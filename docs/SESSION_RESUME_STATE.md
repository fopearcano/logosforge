...
- The remote (`git ls-remote origin`) currently has:
  - `claude/setup-storyplanner-app-5cVxF` → **`d11aad9`** (the *same commit* as local HEAD)
...
- Local remote-tracking refs: `origin/claude/sweet-sagan-0Zwgc` = `d11aad9` (== HEAD), `origin/claude/setup-storyplanner-app-5cVxF` = `fc921a3` (stale from clone).
- **Net:** local `sweet-sagan` and remote `setup-storyplanner-app` point at the **same content** (`d11aad9`); only the branch *name* differs. Pushing `claude/sweet-sagan-0Zwgc` will **create** that branch on the remote (no upstream is currently set).
- The branch named in the task header (`setup-storyplanner-app-5cVxF`) and the branch named in the dev-requirements (`sweet-sagan-0Zwgc`) are therefore the same checkpoint — there is **no content divergence** between them (`git log A..B` is empty in both directions for the shared history; `sweet-sagan` is a strict superset that the remote `setup-storyplanner` has since caught up to).

---

## 2. Runtime / entrypoint

- **Launch command:** `python run.py` (from repo root). Also `python -m storyplanner.diagnostics` prints the runtime report with no GUI.
- **Source resolution:** **local source**, *not* an installed/stale package. Verified by `storyplanner.diagnostics`:
  ```
  package         : /home/user/storyplanner/storyplanner/__init__.py
  main_window     : /home/user/storyplanner/storyplanner/ui/main_window.py
  manuscript view : storyplanner.ui.writing_core_view.WritingCoreView  <- .../writing_core_view.py
  outline view    : storyplanner.ui.plan_view.PlanView                 <- .../plan_view.py
  timeline view   : storyplanner.ui.plot_timeline_view.PlotTimelineView<- .../plot_timeline_view.py
  commit          : d11aad9
  ```
- **App factory:** `storyplanner/app.py::create_app()` → `MainWindow(db, project_id)`; single SQLite DB at `storyplanner.db` (CWD). Lands on **Projects** at startup (`show_initial_section`).
...
| Visible/runtime marker | ✅ | `setObjectName("manuscript_target_writing_page_view")` (`:1340`); dev badge via `STORYPLANNER_DEV_MARKERS=1` |
...
- **Status: fix is present in code** (commit `c35912d`), at `_do_new_project` (`:2477`):
  ...
  - Optional `STORYPLANNER_DEBUG_PROJECT=1` logs window state at each stage.
