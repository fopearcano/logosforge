...
`storyplanner/auto_link.py` scans the manuscript text per scene and produces four kinds of `Suggestion`:
...
3. The banner offers three actions:
   ...
   - **Ignore** — persists the suggestion's stable `entity_key` to `~/.storyplanner/settings.json` under `"auto_link_ignored"` so it never returns.
