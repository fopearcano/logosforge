...
- **One shared AI backend** — `storyplanner.assistant.chat_completion` +
  `storyplanner.providers.build_active_provider`. There is no second chat backend.
...
- **One data model** — `storyplanner.db` + `storyplanner.story_structure` (canonical
