...
from storyplanner.screenplay_blocks import parse_screenplay_text, ScreenplayBlock
from storyplanner import screenplay_setup_payoff as spx
from storyplanner import screenplay_subtext as sub
...
def test_setup_payoff_report_export():
    from storyplanner.export import export_setup_payoff_report_json
    db = Database()
    pid = _project_with_gun(db)
...
def test_subtext_report_export():
    from storyplanner.export import export_subtext_report_json
    db = Database()
    pid = db.create_project("Film", narrative_engine="screenplay").id
...
def test_existing_exports_unbroken():
    from storyplanner.export import export_json, export_screenplay
    db = Database()
    pid = _project_with_gun(db)
