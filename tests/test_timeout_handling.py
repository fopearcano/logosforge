...
class TestDefaultTimeoutForProvider:
    def test_local_provider_lm_studio(self):
        from storyplanner.assistant import default_timeout_for_provider
...
class TestGetConfiguredTimeout:
    def test_auto_zero_uses_provider_default_local(self):
        from storyplanner.assistant import get_configured_timeout
        with patch("storyplanner.settings.get_manager") as mock_mgr:
...
class TestIsTransient:
    def test_socket_timeout_is_transient(self):
        from storyplanner.assistant import _is_transient
...
class TestChatCompletionRetry:
    @patch("storyplanner.assistant.get_configured_timeout", return_value=120)
    @patch("storyplanner.assistant._detect_response_language", return_value="en")
    @patch("storyplanner.assistant._openai_completion")
    ...
    @patch("storyplanner.assistant.time.sleep")
    @patch("storyplanner.assistant.get_configured_timeout", return_value=120)
    @patch("storyplanner.assistant._detect_response_language", return_value="en")
    @patch("storyplanner.assistant._openai_completion")
    ...
    @patch("storyplanner.assistant.time.sleep")
    @patch("storyplanner.assistant.get_configured_timeout", return_value=60)
    @patch("storyplanner.assistant._detect_response_language", return_value="en")
    @patch("storyplanner.assistant._openai_completion")
...
class TestTimeoutErrorMessages:
    @patch("storyplanner.assistant.time.sleep")
    @patch("storyplanner.assistant.get_configured_timeout", return_value=300)
    @patch("storyplanner.assistant._detect_response_language", return_value="en")
    @patch("storyplanner.assistant._openai_completion")
...
class TestTestConnectionTimeout:
    @patch("storyplanner.assistant.chat_completion")
...
class TestAssistantWorkerTimeout:
    def test_worker_stores_timeout(self):
        from storyplanner.providers import ProviderConfig
        from storyplanner.ui.assistant_view import _AssistantWorker
        provider = ProviderConfig(name="LM Studio", base_url="http://localhost:1234/v1")
