...
from storyplanner.version_manager import (
    MAX_VERSIONS,
    VersionManager,
    _version_dir,
)
...
def _with_tmp_versions(project_id):
    ...
    return patch("storyplanner.version_manager.VERSIONS_DIR", tmp), tmp
...
def test_periodic_timer_interval():
    ...
    db, proj = _make_project()
    vm = VersionManager(db, proj.id)
    from storyplanner.version_manager import SNAPSHOT_INTERVAL_MS
