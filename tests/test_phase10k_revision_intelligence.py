...
from storyplanner.revision_intelligence.diff import create_scene_diff
...
def test_psyke_direct_match_confirmed():
    from storyplanner.revision_intelligence.psyke_impact import detect_psyke_impact
    db, pid, s1, s2 = _film()
...
def test_scene_dep_confirmed_story_link():
    from storyplanner.revision_intelligence.scene_impact import detect_scene_impacts
    from storyplanner.screenplay_graph import confirm_candidate
    db, pid, s1, s2 = _film()
...
def test_scene_dep_capped():
    from storyplanner.revision_intelligence.scene_impact import (
...
def test_impact_map_includes_production_when_active():
    import storyplanner.screenplay_production as P
    db, pid, s1, s2 = _film()
