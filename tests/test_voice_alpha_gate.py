...
from storyplanner.voice.types import TranscriptSegment, VoiceSettings
...
def test_full_pipeline_dictate_correct_commit_undo():
    ...
    from storyplanner.voice.commit_router import T_NOTE
...
def test_app_close_while_recording_stops_session_safely():
    from storyplanner.voice.types import VoiceStatus
...
def test_latency_guardrails_with_mock_backend():
    ...
    from storyplanner.voice.setup import PERFORMANCE_PROFILES
...
def test_exports_and_diagnostics_contain_no_voice_data():
    ...
    from storyplanner.voice.setup import diagnostics_summary
...
def test_voice_modules_import_clean_without_optional_deps():
    ...
    for module in ("types", "silence_detector", "audio_buffer",
        ...
        importlib.import_module(f"storyplanner.voice.{module}")
