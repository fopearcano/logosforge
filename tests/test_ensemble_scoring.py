...
class TestScoreWithHeuristic:
    ...
    def test_matches_compute_factors(self):
        from storyplanner.quantum_outliner.scoring import compute_factors
        wf = Wavefunction.new(anchor="test")
...
class TestScoreWithLLM:
    @patch("storyplanner.quantum_outliner.llm_evaluator.chat_completion", side_effect=_mock_chat_success)
    ...
    @patch("storyplanner.quantum_outliner.llm_evaluator.chat_completion", side_effect=_mock_chat_failure)
    def test_returns_none_on_failure(self, mock_cc):
        b = _branch("a", "Test Branch", desc="something happens")
        result = score_with_llm(b, "story context")
        assert result is None

    @patch("storyplanner.quantum_outliner.llm_evaluator.chat_completion",
