...
from storyplanner import screenplay_output_styles as sos
...
def _film(db, *, title=True):
    ...
    db.create_scene(pid, "Vault",
                    ...
                    summary="x")
    if title:
        from storyplanner.screenplay_render import set_title_page
...
def test_docx_export_creates_document(tmp_path):
    from storyplanner.export import export_screenplay_docx
    db = Database()
    pid = _film(db)
...
def test_docx_no_db_mutation_no_llm(tmp_path, monkeypatch):
    import storyplanner.assistant as assistant
    from storyplanner.export import export_screenplay_docx
    calls = []
...
def test_docx_missing_dependency_handled(monkeypatch, tmp_path):
    import storyplanner.screenplay_docx_export as dx
...
def test_pdf_export_writes_pdf(tmp_path):
    from storyplanner.export import export_screenplay_pdf
    db = Database()
    pid = _film(db)
...
def test_fdx_gated_without_acknowledgement():
    from storyplanner.export import export_screenplay_fdx_experimental
    db = Database()
    pid = _film(db)
...
def test_validation_json_export():
    from storyplanner.export import export_screenplay_output_validation_json
    db = Database()
    pid = _film(db)
...
def test_fountain_and_markdown_still_separate_and_work():
    from storyplanner.export import export_screenplay_fountain, export_markdown
    db = Database()
    pid = _film(db)
...
def test_novel_export_unbroken():
    from storyplanner.export import export_fountain, export_json
    db = Database()
    pid = db.create_project("Book", narrative_engine="novel").id
