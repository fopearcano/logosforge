...
from storyplanner import screenplay_fountain as sf
...
def test_generic_markdown_export_separate_and_unbroken():
    from storyplanner.export import export_markdown, export_screenplay_fountain
    db = Database()
    pid = _film(db)
...
def test_fountain_validation_json_export():
    from storyplanner.export import export_fountain_validation_json
    db = Database()
    pid = _film(db)
...
def test_assistant_export_block_switches_for_non_fountain_target():
    from storyplanner.assistant_context_policy import gather_injected_context
    from storyplanner.screenplay_render import set_export_prefs
    db = Database()
    pid = _film(db)
    sid = db.get_all_scenes(pid)[0].id
