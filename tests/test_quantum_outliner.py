...
from storyplanner.quantum_outliner.collapse import CollapseError, collapse, load_archive
from storyplanner.quantum_outliner.possibilities import (
    _parse_branches,
    _stub_branches,
    generate_possibilities,
)
from storyplanner.quantum_outliner.psyke_adapter import (
    apply_collapse,
    find_entry_by_name,
    gather_psyke_brief,
)
from storyplanner.quantum_outliner.state import NarrativeState
...
class TestCollapse:
    ...
    def test_collapse_archive_persists(self, db, project, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "storyplanner.quantum_outliner.collapse.CONFIG_DIR", tmp_path
        )
        wf = Wavefunction.new(anchor="x", branches=[
            Branch.new(title="A", description="a"),
            Branch.new(title="B", description="b"),
...
class TestRelativity:
    def test_reframe_returns_text(self, db, project):
        with patch(
            "storyplanner.quantum_outliner.relativity.chat_completion"
        ) as mock:
            mock.return_value = (
