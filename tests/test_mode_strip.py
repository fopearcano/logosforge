...
from storyplanner.adaptive_mode import AIMode, ModeResult, StoryStage, HealthState
from storyplanner.db import Database
from storyplanner.ui import theme
from storyplanner.ui.mode_strip import ModeStrip, _MODE_HINTS, _MODE_COLORS
...
def test_mode_context_uses_override():
    from storyplanner.adaptive_mode import mode_context_block, _MODE_DESCRIPTIONS
    db, proj = _make_project()
    c = db.create_character(proj.id, "A")
    for i in range(3):
        db.create_scene(proj.id, f"S{i}", character_ids=[c.id])
